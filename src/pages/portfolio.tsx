import { useEffect, useState } from "react";

export default function Portfolio() {
  const [videoElement, setVideoElement] = useState(null);

  useEffect(() => {
    const vid = document.getElementById("video");
    setVideoElement(vid);
  }, []);

  useEffect(() => {
    if (videoElement) {
      let playCount = 0;

      const playXTimes = (x) => {
        videoElement.load();
        videoElement.playbackRate = 1;

        videoElement.onended = function () {
          if (playCount === 2) {
            setVideoElement(null);
          }

          while (playCount < 2) {
            videoElement.play();
            playCount++;
          }
        };
      };

      playXTimes(2);
    }
  }, [videoElement]);

  return (
    <div className="border flex flex-col">
      <h1 className="text-8xl font-Jaldi text-slate-800 m-auto my-8 ">
        PORTFOLIO
      </h1>
      <section className="flex flex-row gap-20 font-Jaldi text-slate-800 p-8">
        <div className="FIRST-ITEM relative">
          <video id="video" autoPlay muted className="w-170 rounded-xl">
            <source src="/videos/fischer-movie.mov" type="video/mp4" />
            <p>Your browser does not support HTML video.</p>
          </video>
          {videoElement === null ? (
            <button
              onClick={(e) => {
                setVideoElement(e.target.previousSibling);
              }}
              className="flex items-center justify-center absolute w-20 h-20 bg-teal-600 rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
            >
              <span className=" pointer-events-none material-symbols-outlined material-icons md-36 text-white">
                play_arrow
              </span>
            </button>
          ) : null}
        </div>

        <div className="flex flex-col w-1/2 gap-4">
          <div className="flex flex-row items-center">
            <p className="text-6xl mr-4">FISCHER</p>
            <div className="flex flex-row gap-1">
              <a
                href="https://faction-fischer.vercel.app/"
                target="_blank"
                rel="noreferrer noopener"
                title="Fischer"
                className="bg-slate-800 h-10 w-10 rounded-full flex items-center justify-center hover:bg-teal-600"
              >
                <span className="pointer-events-none material-symbols-outlined text-white">
                  language
                </span>
              </a>
              <a
                href="https://chrome.google.com/webstore/detail/fischer-fact-checker/bofgempaanegkhlgmkmoomjkfplahnib"
                target="_blank"
                rel="noreferrer noopener"
                title="Fischer Extension"
                className="bg-slate-800 h-10 w-10 rounded-full flex items-center justify-center hover:bg-teal-600"
              >
                <span className="pointer-events-none material-symbols-outlined text-white">
                  extension
                </span>
              </a>
              <a
                href="https://github.com/dmcenroe"
                target="_blank"
                rel="noreferrer noopener"
                title="github"
              >
                <svg
                  viewBox="0 0 100 100"
                  width={40}
                  height={40}
                  className="fill-slate-800 hover:fill-teal-600"
                >
                  <path d="M48.854 0C21.839 0 0 22 0 49.217c0 21.756 13.993 40.172 33.405 46.69 2.427.49 3.316-1.059 3.316-2.362 0-1.141-.08-5.052-.08-9.127-13.59 2.934-16.42-5.867-16.42-5.867-2.184-5.704-5.42-7.17-5.42-7.17-4.448-3.015.324-3.015.324-3.015 4.934.326 7.523 5.052 7.523 5.052 4.367 7.496 11.404 5.378 14.235 4.074.404-3.178 1.699-5.378 3.074-6.6-10.839-1.141-22.243-5.378-22.243-24.283 0-5.378 1.94-9.778 5.014-13.2-.485-1.222-2.184-6.275.486-13.038 0 0 4.125-1.304 13.426 5.052a46.97 46.97 0 0 1 12.214-1.63c4.125 0 8.33.571 12.213 1.63 9.302-6.356 13.427-5.052 13.427-5.052 2.67 6.763.97 11.816.485 13.038 3.155 3.422 5.015 7.822 5.015 13.2 0 18.905-11.404 23.06-22.324 24.283 1.78 1.548 3.316 4.481 3.316 9.126 0 6.6-.08 11.897-.08 13.526 0 1.304.89 2.853 3.316 2.364 19.412-6.52 33.405-24.935 33.405-46.691C97.707 22 75.788 0 48.854 0z" />
                </svg>
              </a>
            </div>
          </div>
          <div className="TECHS-USED grid grid-cols-3 w-max gap-x-3 gap-y-2">
            <div className=" text-black bg-gray-100 rounded-lg flex items-center justify-center px-7 h-6 border border-black">
              Next.js
            </div>
            <div className=" text-blue-700 bg-blue-100 rounded-lg flex items-center justify-center h-6 border border-blue-700">
              Typescript
            </div>
            <div className=" text-red-500 bg-rose-100 rounded-lg flex items-center justify-center h-6 border border-red-500">
              OpenAI
            </div>
            <div className=" text-green-700 bg-green-100 rounded-lg flex items-center justify-center h-6 border border-green-700">
              supabase
            </div>
            <div className=" text-indigo-700 bg-indigo-100 rounded-lg flex items-center justify-center h-6 border border-indigo-700">
              Prisma
            </div>
            <div className=" text-sky-700 bg-sky-100  rounded-lg flex items-center justify-center h-6 border border-sky-700">
              tailwind
            </div>
          </div>
          <p className="text-xl mt-8">
            Fischer leverages emerging AI technology to provide a first line of
            defense against misinformation. We combine that data with input from
            real users, and topic experts to determine the validity of
            statements across the internet.
          </p>
          <p className="text-xl">
            Fischer helps you identify the validity of information in real time
            as you’re browsing websites. Any piece of information that has been
            submitted by users will be highlighted green, yellow, or red,
            indicating a true, subjective, or false statement. Hover over these
            sections on the website for more information.
          </p>
        </div>
      </section>
    </div>
  );
}
